...
// Бюджети часу. maxDuration = 60 с; за цією межею Vercel просто вбиває
// функцію — у логах це статус 0, а не помилка, тому проблему легко проґавити.
//
// Перевірка часу стоїть ПЕРЕД КОЖНИМ зображенням, а не лише на вході в товар.
// Оскільки в кожного запиту свій таймаут (10 с), максимальний виліт за точку
// перевірки — один таймаут. Звідси й числа:
//   новий товар починаємо, поки не минуло 38 с  → найгірше 38 + 10 = 48 с
//   додаткові фото тягнемо, поки не минуло 42 с → найгірше 42 + 10 = 52 с
// Лишається 8 секунд на запис результату в базу — з запасом.
const START_BUDGET_MS = 38_000;
...
export async function GET(request: Request) {
  ...
  try {
    ...
    for (const row of rows as Row[]) {
      // За новий товар беремось лише якщо встигнемо його завершити.
